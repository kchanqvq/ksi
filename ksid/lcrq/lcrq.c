...
void ksiLCRQInit(KsiLCRQ * q, int nprocs)
{
  ...
  q->head = rq;
  q->tail = rq;
  q->ebr = ksiDynamicEBRCreate(nprocs);
}
...
void ksiLCRQEnqueue(KsiLCRQ * q, KsiLCRQProducerHandle * handle, void * v) {
    ...
    if (tail_index(rq->tail) <= h + 1) {
      ...
      if (tail_index(rq->tail) <= h + 1) {
        if (CAS(&q->head, &rq, next)) {
          ksiDynamicEBRRetire(q->ebr, rq, tid);
        }
      }
    }
  }
...
void ksiLCRQProducerHandleInit(KsiLCRQ * q, KsiLCRQProducerHandle * th){
  th->olId = ksiDynamicEBROverlookerRegister(q->ebr);
  th->next = NULL;
}
void ksiLCRQProducerHandleDestroy(KsiLCRQ *q, KsiLCRQProducerHandle *h){
  ksiDynamicEBROverlookerUnregister(q->ebr,h->olId);
}
