  ...
  (class pasteboard%
    ...
    (define/augment (after-move-to snip x y dragging)
      (when (equal? node-snip-class (send snip get-snipclass))
        (let-values ([(dx dy) (send this editor-location-to-dc-location x y)])
          (send snip adjust-all-wire-location dx dy))))
    (define/override (on-event event)
      (define (handle-port-event in out) (let ([s (send this find-snip
                                           ...
                                           (when s (send this get-snip-location s x y)
                                                 (let-values ([(dx dy) (send this editor-location-to-dc-location (unbox x) (unbox y))])
          ...
          (cond [(send event moving?)
                 (begin
                   (if (get-field inlet-node incomplete-wire)
                     (begin
                       (set-field! outlet-x incomplete-wire (send event get-x))
                       (set-field! outlet-y incomplete-wire (send event get-y)))
                     (begin
                       (set-field! inlet-x incomplete-wire (send event get-x))
                       (set-field! inlet-y incomplete-wire (send event get-y))))
                   (send incomplete-wire update this)
                   (handle-port-event (lambda (port-id port s dx dy) (send s set-highlight-inlet port-id))
                                      (lambda (port-id port s dx dy) (send s set-highlight-outlet port-id))))]
                [(send event button-down?)
                 (handle-port-event (lambda (port-id port s dx dy) (send s adjust-wire-location dx dy incomplete-wire port-id 'inlet)
                                    ...
                                    (lambda (port-id port s dx dy) (send s adjust-wire-location dx dy incomplete-wire port-id 'outlet)
...
(define wire-snip%
  ...
  (define/override (get-extent dc x y
    ...
    (define/override (draw dc x y left top right bottom dx dy draw-caret)
      ...
      (define midy (/ (+ inlet-y outlet-y) 2))
      (send wp move-to inlet-x inlet-y)
      (send wp curve-to (+ (* outlet-x wire-curve-ratio) (* inlet-x wire-curve-coratio)) midy (+ (* outlet-x wire-curve-coratio) (* inlet-x wire-curve-ratio)) midy outlet-x outlet-y )
