...
static struct rbnode *rbtree_next(struct rbtree *tree, struct rbnode *node){
	struct rbnode *root = tree->root;
	struct rbnode *sentinel = tree->sentinel;
...
void ksiRBTreeInit(KsiRBTree *tree){
	KsiRBNode *node = (KsiRBNode *)malloc(sizeof(KsiRBNode));
	rbtree_node_init(node);
	rbtree_black(node);
	tree->root = node;
	tree->sentinel = node;
...
KsiRBNode *ksiRBTreeNext(KsiRBTree *tree,KsiRBNode *n){
	return rbtree_next(tree, n);
}

void ksiRBNodeDestroy(KsiRBTree *t,KsiRBNode *root){
	if(root == t->sentinel)
		return;
	if(root->left){
		ksiRBNodeDestroy(t,root->left);
	}
	if(root->right){
		ksiRBNodeDestroy(t,root->right);
	}
	free(root);
}
void ksiRBTreeDestroy(KsiRBTree *t){
	ksiRBNodeDestroy(t,t->root);
	free(t->sentinel);
}
